...
($VERSION) = ($PERVERSION = "CLC-INTERCAL/Base INTERCAL/Parser.pm 1.-94.-2.4") =~ /\s(\S+)$/;
...
sub _gencode {
	...
	if ($type eq '*') {
	    my $start1 = $start & 0x000000FF;
 		my $start2 = ($start & 0x0000FF00) >> 8;
 		my $start3 = ($start & 0x00FF0000) >> 16;
 		my $start4 = ($start & 0xFF000000) >> 24;
 		my $length1 = $length & 0x000000FF;
 		my $length2 = ($length & 0x0000FF00) >> 8;
 		my $length3 = ($length & 0x00FF0000) >> 16;
 		my $length4 = ($length & 0xFF000000) >> 24;
 	    $code .= pack('C*', (
 			0x7f, $start2, $start1, 0x7f, $start4, $start3, 
 			0x7f, $length2, $length1, 0x7f, $length4, $length3,
 			0x7f, ($junk & 0xff00) >> 8, $junk & 0xff, 
 			0x7f, ((length $uses) & 0xff00) >> 8, (length $uses) & 0xff));
	    $code .= $uses;
	    next;
	}
    }
