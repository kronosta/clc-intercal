...
($VERSION) = ($PERVERSION = "CLC-INTERCAL/Base INTERCAL/Object.pm 1.-94.-2.3") =~ /\s(\S+)$/;
...
sub write {
	...
	my $source = '';
	if (! $discard) {
	    my $slen = unpack('v', $fh->write_binary(2));
	    $source = $fh->write_binary($slen);
	}
	my ($length, $ncptr, $uflags, $codelen) = unpack('vvvv', $fh->write_binary(8));
	my $code = $fh->write_binary($codelen);
	my @cptr;
	while (@cptr < $ncptr) {
	    my ($sptr, $np) = unpack('vv', $fh->write_binary(4));
	    my @sptr;
	    for (my $p = 0; $p < $np; $p++) {
		...
		my $ru;
		if (! $discard) {
		    my $rl = unpack('v', $fh->write_binary(2));
...
sub unit_code {
    @_ == 2 || @_ == 5 or croak "Usage: OBJECT->unit_code(UNIT [, SOURCE, LENGTH, NEWCODE])";
    my $obj = shift;
    my $unit = shift;
    $unit =~ /^\d+$/ && $unit <= @{$obj->{units}} or croak "Invalid UNIT $unit";
    my @oldcode = @{$obj->{units}[$unit] || []};
    if (@_) {
	my ($source, $length, $origcode) = @_;
	length($source) > 0xffff || $length > 0xffff
	    and faint(SP_INDIGESTION);
	my $optcode = $obj->{optimiser}->optimise($origcode);
	my ($code, $cptr, $discard, $uflags) = _setcode($obj->{bug}, $obj->{flags}, $optcode);
